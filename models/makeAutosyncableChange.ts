...
if (!fileName || !stringToReplace) {
  throw new Error(
    "Usage: node script.ts <fileName> <stringToReplace> [mainBranchName]"
  );
}
...
try {
  ...
  execSync(
    `gh pr create --base ${mainBranchName} --head temp-autosync-branch-${dateOfToday} --title "Auto Sync ${stringToReplace}" --body "Auto Sync ${stringToReplace}"`
  );
  execSync(`git checkout ${mainBranchName}`);
  execSync("git pull");
  console.log(
    `Pull request created: https://github.com/user/repository/pull/new/temp-autosync-branch-${dateOfToday}`
  );
} catch (error) {
  console.error(error);
  process.exit(1);
